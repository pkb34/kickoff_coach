...
- `self_reported_happiness_index = 10 × the student's 0–10 seven-day happiness rating`. This is a display conversion of a direct answer, **not** a validated wellbeing score. If the student skips the rating, display `Not reported`.
...
The existing four-role demo rule remains in `engine.assign_local_demo_personality()`. `football_matches.match_football_identity(role)` attaches one national-team style and player analogy to each role. The mappings are editorial and cite FIFA/UEFA sources for the football descriptions. They are not a scientific match between a student and an athlete.

## Optional Gemini path

Only `gemini_gateway.py` makes a network request. The result page calls it **after the student clicks** `Ask The Gaffer for Gemini Briefing`:
...
3. Update `football_matches.py` to add more teams or players. Keep football-source links separate from the student-match explanation.
